			 ...
			 $selQry = "select * from tbl_user u inner join tbl_place p on p.place_id=u.place_id inner join tbl_district d on d.district_id=p.district_id";
				 ...
				 $rs = $conn->query($selQry);
                   while ($data = $rs->fetch_assoc()) 
				   {
					   ...
					   $selQry2 = "select count(booking_id) as qty from tbl_booking b inner join tbl_user u on u.user_id=b.user_id inner join tbl_place p on p.place_id=u.place_id inner join tbl_district d on d.district_id=p.district_id where b.user_id='".$data["user_id"]."'";
					  //echo $selQry2;
